...
Revision ID: ebca6e35d395
Revises: 
Create Date: 2025-11-18 10:06:34.499863
...
def upgrade():
    ...
    with op.batch_alter_table('ai_generator_notepost', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('ai_generator_blogpost_user_id_18b258a5'))

    op.drop_table('ai_generator_notepost')
    ...
    with op.batch_alter_table('contract', schema=None) as batch_op:
        batch_op.create_unique_constraint(None, ['contract_id'])

    with op.batch_alter_table('product', schema=None) as batch_op:
        batch_op.alter_column('call_limit_type',
...
def downgrade():
    ...
    op.create_table('ai_generator_notepost',
    sa.Column('id', sa.BIGINT(), sa.Identity(always=False, start=1, increment=1, minvalue=1, maxvalue=9223372036854775807, cycle=False, cache=1), autoincrement=True, nullable=False),
    sa.Column('youtube_title', sa.VARCHAR(length=300), autoincrement=False, nullable=False),
    sa.Column('youtube_link', sa.VARCHAR(length=200), autoincrement=False, nullable=False),
    sa.Column('generated_content', sa.TEXT(), autoincrement=False, nullable=False),
    sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=False),
    sa.Column('user_id', sa.INTEGER(), autoincrement=False, nullable=False),
    sa.ForeignKeyConstraint(['user_id'], ['auth_user.id'], name=op.f('ai_generator_blogpost_user_id_18b258a5_fk_auth_user_id'), initially='DEFERRED', deferrable=True),
    sa.PrimaryKeyConstraint('id', name=op.f('ai_generator_blogpost_pkey'))
    )
    with op.batch_alter_table('ai_generator_notepost', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ai_generator_blogpost_user_id_18b258a5'), ['user_id'], unique=False)
